...
if(bbDom){
  ...
  btn.addEventListener("click", function () {
    ...
    updateHTMl(nextDom)
    if(nextLength < limit){ //è¿”å›žæ•°æ®æ¡æ•°å°äºŽé™åˆ¶æ¡æ•°ï¼Œéšè—
...
function getNextList(){
  ...
  fetch(bbUrl).then(res => res.json()).then( resdata =>{
    ...
    page++
    offset = limit*(page-1)
    if(nextLength < 1){ //è¿”å›žæ•°æ®æ¡æ•°ä¸º 0 ï¼Œéšè—
      document.querySelector("button.button-load").remove()
      return
    }
  })
